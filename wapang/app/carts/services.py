...
class CartService:
    ...
    def update_cart(
        ...
        cart_product = self.cart_repository.get_cart_product(user.id, request.item_id)
        ...
        if request.quantity > 0:
            if cart_product:
                ...
                self.cart_repository.add(new_cart_product)
        else:
            if cart_product:
                self.cart_repository.delete(cart_product)
    
        all_cart_products = self.cart_repository.get_all_cart_products_with_details(user.id)
    ...
    def get_cart(self, user: User) -> tuple[dict[str, CartDetails], int]:
    ...
    def clear_cart(self, user: User) -> None:
        self.cart_repository.delete_all_cart_products(user.id)
        
    def checkout(self, user: User) -> tuple[Order, dict[str, OrderDetails]]:
        all_cart_products = self.cart_repository.get_all_cart_products_with_details(user.id)
