...
class OrderService:
    ...
    def create_order(self, request: OrderCreateRequest, user: User) -> tuple[Order, dict[str, OrderDetails]]:
        if not request.items:
            raise EmptyItemListException()
        
        item_ids = [item.item_id for item in request.items]
        products = self.product_repository.get_items_by_ids(item_ids)
        ...
        self.order_repository.session.flush()
    ...
    def get_order(self, order_id: str, user: User) -> tuple[Order, dict[str, OrderDetails]]:
        ...
        order_products = self.order_repository.get_order_products_with_details(order_id)
    ...
    def update_order(self, order_id: str, request: OrderPatchRequest, user: User) -> tuple[Order, dict[str, OrderDetails]]:
        order = self.order_repository.get_order_by_id(order_id)
        ...
        if request.status == OrderStatus.CANCELED:
            ...
            order_products = self.order_repository.get_order_products_for_restock(order_id)
            for op in order_products:
                if op.product:
        ...
        self.order_repository.add_objects_to_session(objects_to_save)
